...
init //hooking to game to make memorywatchers
{
	...
	if (modules.First().ToString() == "MCC-Win64-Shipping.exe")
	{
		...
		if (version == "1.2904.0.0")
		{
			...
			vars.watchers_odst = new MemoryWatcherList() {
				(vars.ODST_levelname = new StringWatcher(new DeepPointer(0x03F7BA50, 0xA8, 0x20872f5), 4)), 
		...
		else if (version == "1.2862.0.0")
		{
			...
			vars.watchers_h1 = new MemoryWatcherList() {
				...
				(vars.H1_IGT = new MemoryWatcher<uint>(new DeepPointer(0x03F7BAD0, 0x8, 0x2E7A364)) { FailAction = MemoryWatcher.ReadFailAction.SetZeroOrNull}),
			...
			vars.watchers_odst = new MemoryWatcherList() {
				(vars.ODST_levelname = new StringWatcher(new DeepPointer(0x03F7BAD0, 0xA8, 0x20872f5), 4)), 
		...
		else if (version == "1.2645.0.0")
		{
			...
			vars.watchers_odst = new MemoryWatcherList() {
				(vars.ODST_levelname = new StringWatcher(new DeepPointer(0x03B80E98, 0xA8, 0x2079d85), 4)),
		...
		else if (version == "1.2448.0.0")
		{
			...
			vars.watchers_odst = new MemoryWatcherList() {
				(vars.ODST_levelname = new StringWatcher(new DeepPointer(0x3A24FF8, 0xA8, 0x1D25187), 4)), 
	...
	else if (modules.First().ToString() == "MCC-Win64-Shipping-WinStore.exe" || modules.First().ToString() == "MCCWinStore-Win64-Shipping.exe")
	{
		if (version == "1.2904.0.0")
		{
			...
			vars.watchers_odst = new MemoryWatcherList() {
				(vars.ODST_levelname = new StringWatcher(new DeepPointer(0x03E1F540, 0xA8, 0x20872f5), 4)), 
...
{ 	
	...
	settings.Add("sqsplit", false, "Split when loading a new level");
...
update 
{
	...
	else if (vars.menuindicator.Current == 7)
	{
		...
		switch (test)
		{
			...
			if (settings["ILmode"]) {vars.watchers_h3IL.UpdateAll(game);}
			break;
	...
	if (timer.CurrentPhase == TimerPhase.Running)
	{
		...
		if (vars.partialreset)
		{
			switch (test)
			{
				...
				case 5:
				if (vars.ODST_levelname.Current != "h100") {vars.dirtybsps_long.Clear();}
				break;
		...
		if (vars.menuindicator.Current == 7 && vars.startedlevel == "000")
		{
			switch (test)
			{
				case 0:
		...
		if (!vars.multigamepause)
		{
			...
			if (vars.menuindicator.Current == 7 && (settings["IGTmode"] || !(test == 0 || (test == 1 && !settings["ILmode"]))))
			{
				...
				switch (test)
				{
					...
					if (settings["ILmode"])
					{
						IGT = vars.H3_IGT.Current;
						IGTold = vars.H3_IGT.Old;
						tickrate = 60;
					...
					case 5:
					IGT = vars.ODST_IGT.Current;
					IGTold = vars.ODST_IGT.Old;
					tickrate = 60;
					level = vars.ODST_levelname.Current;
					break;

					case 6:
					IGT = vars.HR_IGT.Current;
					IGTold = vars.HR_IGT.Old;
					tickrate = 60;
			...
			if (!settings["ILmode"])
			{
				...
				if (settings["sqsplit"] && vars.gameindicator.Current == 0)
				{
					...
					else if (vars.forcesplit2 == false && (vars.H1_levelname.Current != vars.levelloaded) && 
					(vars.H1_levelname.Current == "a10" 
						|| vars.H1_levelname.Current == "a30" 
						|| vars.H1_levelname.Current == "a50" 
						|| vars.H1_levelname.Current == "b30" 
		...
		else if (vars.multigamepause)
		{
			if (vars.menuindicator.Current == 7) 
			{
				switch (test)
				{
					...
					case 5: //ODST
					if (vars.ODST_levelname.Current == "c100" && vars.ODST_IGT.Current > 15 && vars.ODST_IGT.Current < 30) //there was a bsp check here; do I still need it?
					{
						vars.multigamepause = false;
					}
					break;
...
{	
	...
	if (vars.menuindicator.Current == 7) 
	{
		...
		switch (test)
		{
			...
			//ODST
			case 5:
			if ((settings["ILmode"] || vars.ODST_levelname.Current == "c100") && vars.ODST_IGT.Current > 10 && vars.ODST_IGT.Current < 30)
			{
				vars.startedlevel = vars.ODST_levelname.Current;
				return true;
			}
...
reset
{
	...
	if ((!(settings["Loopmode"])) && vars.menuindicator.Current == 7)
	{
		...
		switch (test)
		{
			...
			} else return (vars.H2_levelname.Current == "00a");
			break;
			
			//H3
			case 2:
			if (vars.H3_levelname.Current == vars.startedlevel && vars.startedgame == 2 && timer.CurrentPhase != TimerPhase.Ended)
			{
				if (settings["ILmode"])
				{
					return (vars.H3_IGT.Current < vars.H3_IGT.Old && vars.H3_IGT.Current < 10);
				}
				else
				{
			...
			//ODST
			case 5:
			if (vars.ODST_levelname.Current == vars.startedlevel && vars.startedgame == 5 && timer.CurrentPhase != TimerPhase.Ended)
			{
				return ((vars.ODST_IGT.Current < vars.ODST_IGT.Old && vars.ODST_IGT.Current < 10) || (vars.stateindicator.Current == 44 && vars.ODST_IGT.Current == 0));
			} 
			break;
			...
			{
				return ((vars.HR_IGT.Current < vars.HR_IGT.Old && vars.HR_IGT.Current < 10) || (vars.stateindicator.Current == 44 && vars.HR_IGT.Current == 0));
			}
			break;
		}
	}
